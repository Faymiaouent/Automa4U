...
public class REtoNFA {
    ...
    public REtoNFA() throws Exception {
    }
    ...
    class NFA
    {
         ...
         */
        void display() {
            ArrayList<Transitions> transitionstest= new ArrayList<>();
            System.out.println("digraph finitestatemachine { \n size=\"8,5\"");
            System.out.println("rankdir = LR");
            System.out.println("node [shape = doublecircle];" + get_final_state());
            System.out.println("node [shape = point];q;");
            System.out.println("node [shape=circle];");
            System.out.println("q->0");
            for(int i = 0; i < transitions.size(); i++) {
                Transitions temp = transitions.get(i);
                if (!transitionstest.contains(transitions.get(i))) {
                    System.out.println(temp.vertex_from+"->"+temp.vertex_to+"[label="+temp.symbol+"];");
                    transitionstest.add(transitions.get(i));
                }
            }
            System.out.println("}");
        ...
        ArrayList<Character> find_possible_input_symbols(ArrayList<Integer> vertex) {
            ...
            for (int i = 0; i < vertex.size(); i++) {
                ...
                for (int j = 0; j < transitions.size(); j++) {
                    ...
                    if (it.vertex_from == vertex_from && it.symbol != 'ε') {
        ...
        void eclosure(int x, ArrayList<Integer> result, boolean visited[]) //Simple DFS
        {
            ...
            for (int i = 0; i < transitions.size(); i++) {
                ...
                if (it.vertex_from == x && it.symbol == 'ε') {
                    int y = it.vertex_to;
                    if (!visited[y]) {
                        visited[y] = true;
    ...
    NFA concat(NFA a, NFA b)
    {
        ...
        result.set_transition(a.get_final_state(), a.get_vertex_count(), 'ε');//Creating the link; final state of a will link to initial state of b
    ...
    NFA kleene(NFA a)
    {
        ...
        result.set_transition(0, 1, 'ε');//Epsilon transition from S0 to S1
        for(i = 0; i < a.transitions.size(); i++) {
            new_trans = a.transitions.get(i);
            result.set_transition(new_trans.vertex_from + 1, new_trans.vertex_to + 1, new_trans.symbol);//Copy old transitions
        }
        result.set_transition(a.get_vertex_count(), a.get_vertex_count() + 1, 'ε');//Epsilon transition to new final state
        result.set_transition(a.get_vertex_count(), 1, 'ε');//Reverese epsilon transition
        result.set_transition(0, a.get_vertex_count() + 1, 'ε');//Forward total epsilon transition
    ...
    NFA or_selection(ArrayList<NFA> selections, int no_of_selections)
    {
        ...
        for(i = 0; i < no_of_selections; i++)
        {
            result.set_transition(0, adder_track, 'ε');//Initial epsilon transition to the first block of 'OR'
            ...
            result.set_transition(adder_track - 1, vertex_count - 1, 'ε');//Add epsilon transition to final state
        }
    ...
    class DFA
    {
        ...
        void set_transition(int vertex_from, int vertex_to, char symbol) {
            ...
            if (!transitions.contains(new_trans)) transitions.add(new_trans);
        }
        ...
        /*void display(){
            ...
            for (int i = 0; i < transitions.size(); i++) {
                for (int j = 0; j < transitions.size(); j++) {
                    if (i!=j){
                        Transitions tri = transitions.get(i);
                        Transitions trj = transitions.get(j);
                        if (tri.vertex_from == trj.vertex_from &&
                            tri.vertex_to == trj.vertex_to && tri.symbol == trj.symbol){
         ...
         */
        String afficherListe(ArrayList<Integer> finalstates){
            String str = " ";
            for (int i = 0; i < finalstates.size() ; i++) {
                str = str+" "+finalstates.get(i);
            }
            return str;
        }
        ...
        void display() {
            Transitions new_trans;
            System.out.println("digraph finitestatemachine { \n size = \"8,5\" \n rankdir = LR; \n node [shape=doublecircle];" +afficherListe(final_states)+";");
            ...
            for(int i = 0; i < transitions.size(); i++) {
                new_trans = transitions.get(i);
                System.out.println(new_trans.vertex_from + "->" + new_trans.vertex_to +
                            //" {" +join(entries.get(new_trans.vertex_to), ",")
                            "[label=" + new_trans.symbol + "];");
            }
            System.out.println("}");
    ...
    public void run(String string) {
        ...
        if (required_dfa.evaluate(eval)) {
            System.out.println(eval + " is accepted by regex " + string);
        } else {
            System.out.println(eval + " is rejected by regex " + string);
        }
